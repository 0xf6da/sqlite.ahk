...
sqlite(db_path,sqlite_path:="sqlite3.dll") { ;sqlite version 3460000
    ...
    if (!hModule) {
        throw Error("failed to load sqlite3.dll using path: " sqlite_path)
    }
    ...
    prepare(_,statement) {
        ...
        if (ret) {
            ...
            switch errmsg {
                case "near `")`": syntax error":
                    errmsg.=", note that trailing commas are not allowed in SQL"
            }
            caller:=Error(,-1)
            throw Error(errmsg "`n(check line " caller.Line " for errors, in file: " caller.File ")")
        }
...
; db:=sqlite(":memory:", A_LineFile "\..\sqlite3.dll")
...
; assertThrows(()=>db.prepare("
; (
; CREATE TABLE t1 (
;    id TEXT PRIMARY KEY,
; `)
; )").all(),"near `")`": syntax error, note that trailing commas are not allowed in SQL" ("`n(check line " Error().Line " for errors, in file: " Error().File ")"))
...
; db.prepare("insert into t1 (id) VALUES (?)").all("111")
; assertThrows(()=>db.prepare("insert into t1 (id) VALUES (?)").all("111"),"UNIQUE constraint failed: t1.id" ("`n(check line " Error().Line " for errors, in file: " Error().File ")"))
